...
if __name__ == '__main__':
    ...
    with open(config['yolo_classes_fname']) as cf:
        classNames = cf.readlines()
    classNames = [c.strip() for c in classNames]
    yolo_classes_to_track = json.loads(config.get('yolo_classes_to_track'))
    trackerSettings = {'tracker': config.get('tracker_to_use', 'kcf'),
                                'class_names': class_names,
                                'obj_disappear_thresh': config.getint('obj_disappear_thresh', 60),
                                'obj_teleport_threshold': config.getfloat('obj_teleport_threshold', 0.4),
    ...
    for feedID in range(num_feeds):
        ...
        for yoloclass in yolo_classes_to_track:
            if yoloclass in class_names:
                ...
                raise ValueError('Class not found in yolo, check yolo_classes_to_track in config file')
